...
def deap_load(data_file,dimention,debaseline):
    ...
    dataset_dir = "deap_shuffled_MWMF_div/" + with_or_without + "_" + arousal_or_valence + "/"
    ...
    datasets = rnn_datasets.reshape(-1, 4, 9, 9).astype('float32')
...
def CapsNet(input_shape, n_class, routings, batch_size):
    ...
    conv1 = layers.Conv2D(filters=64, kernel_size=4, strides=1, padding='same', activation='selu', name='conv1')(x)
    # print(conv1.shape)
    conv2 = layers.Conv2D(filters=128, kernel_size=4, strides=1, padding='same', activation='selu', name='conv2')(conv1)
    conv3 = layers.Conv2D(filters=256, kernel_size=4, strides=1, padding='same', activation='selu', name='conv3')(conv2)
    # conv4 = layers.Conv2D(filters=512, kernel_size=4, strides=1, padding='same', activation='selu', name='conv4')(conv3)
    # conv5 = layers.Conv2D(filters=1024, kernel_size=4, strides=1, padding='same', activation='selu', name='conv5')(conv4)

    # Layer 2: Conv2D layer with `squash` activation, then reshape to [None, num_capsule, dim_capsule]
    primarycaps = PrimaryCap(conv3, dim_capsule=8, n_channels=32, kernel_size=4, strides=1, padding='same')
    ...
    decoder.add(layers.Dense(512, activation='selu', input_dim=16 * n_class))
    decoder.add(layers.Dense(1024, activation='selu'))
...
subjects = ['s22'] #'s01','s02','s03','s04','s05','s06','s07','s08','s09','s10','s11','s12','s13','s14','s15','s16','s17','s18','s19','s20',
    ...
    for dimention in dimentions:
        for subject in subjects:
            ...
            for curr_fold in range(fold):
                ...
                y_pred = eval_model.predict(x_test, batch_size=120)  # batch_size = 100
                ...
                # print(len(y_pred))
                # print(y_test.shape)
                test_acc_fold = np.sum(np.argmax(y_pred[0], 1) == np.argmax(y_test, 1)) / y_test.shape[0]
