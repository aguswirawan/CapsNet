...
for label in ['all']: # ,'arousal','dominance']:#['valence','arousal','dominance']:
    ...
    for sub in subjects:
        print(sub)
        xl = xlrd.open_workbook(r'result_noise_MWMF2_div/sub_dependent_v0/yes/s'+sub+'_'+label+epoch+'/summary_s'+sub+'.xlsx')
        table = xl.sheets()[1]
...
writer = pd.ExcelWriter('result_noise_MWMF2_div_'+label+'.xlsx')
