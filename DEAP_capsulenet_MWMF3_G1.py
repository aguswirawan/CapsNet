...
dataset_name = 'primer' #'deap' # dreamer
...
if __name__ == "__main__":
    ...
    y_train_list = []
    y_test_list = {}
    if dataset_name == 'primer':          # load dreamer data
        for subject in subjects:
            dataset, label, label_name = data_load(subject,dimentions[0],debaseline)
            if 'HAHV' not in label_name:
                label_zeros = np.zeros(label.shape[0])
                label = np.insert(label, 0, label_zeros, axis=1)
            elif 'HALV' not in label_name:
                label_zeros = np.zeros(label.shape[0])
                label = np.insert(label, 1, label_zeros, axis=1)
            elif 'LAHV' not in label_name:
                label_zeros = np.zeros(label.shape[0])
                label = np.insert(label, 2, label_zeros, axis=1)
            elif 'LALV' not in label_name:
                label_zeros = np.zeros(label.shape[0])
                label = np.insert(label, 3, label_zeros, axis=1)
            X_train_subject, X_test_subject, y_train_subject, y_test_subject = train_test_split(dataset, label, test_size=0.1, shuffle=True, random_state=42,stratify=label)
            X_train_list.append(X_train_subject)
            X_test_dict[subject] = X_test_subject
            y_train_list.append(y_train_subject)
            y_test_list[subject] = y_test_subject
    datasets, labels = np.concatenate(X_train_list), np.concatenate(y_train_list)
    from sklearn.utils import shuffle
    datasets, labels = shuffle(datasets, labels,random_state=42)
    print(datasets.shape)
    print('shape after merging:')
    
    print(labels.shape)

    args.save_dir = args.save_dir + '/' + debaseline + '/' + subject + '_' + dimentions[0] + str(args.epochs)
    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)
    ...
    loss_allfold = []
    test_accuracy_subject_allfold = {subject:np.zeros(shape=[0], dtype=float) for subject in subjects}
    for curr_fold in range(fold):
        ...
        for subject in subjects:
            print(f'Test acc subject {subject}:')
...
def data_load(data_file,dimention,debaseline):
    ...
    arousal_or_valence = dimention
    with_or_without = debaseline # 'yes','not'
    dataset_dir = "amigos_shuffled/deap_shuffled_noise_MWMF_3_div/" + with_or_without + "_" + arousal_or_valence + "/"
...
subjects = ['s01','s02','s03','s04','s05','s06','s07','s08','s09','s10','s11','s12','s13','s14','s15','s16']  #  ['s01','s02','s03','s04','s05','s06','s07','s08','s09','s10','s11','s12','s13','s14','s15','s16']#,'s05']#,'s06','s07','s08']#,'s09','s10','s11','s12','s13','s14','s15','s16'，'s17','s18','s19','s20','s21','s22','s23','s24','s25','s26','s27','s28',]
# subjects = ['s01'] #'s01','s02','s03','s04','s05','s06','s07','s08','s09','s10','s11','s12','s13','s14','s15','s16','s17','s18','s19','s20',
...
if __name__ == "__main__":
    for dimention in dimentions:
        for subject in subjects:
            ...
            parser.add_argument('--debug', default=0, type=int,
                                help="Save weights by TensorBoard")
            parser.add_argument('--save_dir', default='result_noise_MWMF_3_div/sub_dependent_'+ model_version +'/') # other
            parser.add_argument('-t', '--testing', action='store_true',
                                help="Test the trained model on testing dataset")
            parser.add_argument('-w', '--weights', default=None,
                                help="The path of the saved weights. Should be specified when testing")
